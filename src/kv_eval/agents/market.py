...
class _LLMCriterionScore(BaseModel):
    ...
    rationale: str = Field(description="점수 근거 2~3문장 + 바로 위 점수를 주지 않은 이유 1문장")
...
def score(task: ScoreTask, rubrics: dict) -> list[CriterionResult]:
    """rubrics["judge"]["common_prompt"] + rubrics["agents"]["market"]["judge_addon"]
    + prompts/score/market.md 로 LLM 채점. task.evidence만 근거로 쓰고, tech_brief는 배경 참고용."""
